...
- 🧱 **One-click "Prepare server"** — bootstrap a clean Ubuntu host with the whole stack: PHP 8.1–8.5 (fpm+cli) with every Laravel/Filament extension (intl, gd, bcmath, redis, soap…), nginx, MariaDB, Composer, Supervisor, Certbot, Redis, and `/var/www/sites` + firewall — streamed live, idempotent, unreleased PHP versions skipped gracefully
